...
function extractPitchToken(note) {
  ...
  const accidental = cleaned.includes("b") ? "b" : cleaned.includes("#") ? "#" : "";
...
function normalizeChord(rawChord, index) {
  ...
  const notes = Array.isArray(rawChord.notes)
    ? rawChord.notes
        .map((note) => sanitizeDisplayPitch(note))
        .filter(Boolean)
        .slice(0, 6)
    : [];
...
export function normalizePitchClass(note) {
  const canonical = extractPitchToken(note);
  if (!canonical) {
    return null;
  }
...
export function sanitizeDisplayPitch(note) {
  return extractPitchToken(note);
}
